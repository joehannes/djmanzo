...
| 26 | Direct manipulation on the waveform | 🟡 | Four of §26's nine examples ship: the **transition start and end**, the **cue markers**, and the **loop**, whose edges are drawn and dragged on the deck's own lane. The frame under the hand goes to djmanzo, which snaps it to a beat *if the DJ has quantise on*, clamps it into the record and refuses an empty slot; the lane draws what comes back. It goes through the same action the pads send, so it lands in the session log and is remembered with the record. Still numbers in panels: phrase markers, saved loops, beat jumps, stem regions, AI suggestions |
