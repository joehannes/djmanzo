...
| 26 | Direct manipulation on the waveform | 🟡 | Five of §26's nine examples ship: the **transition start and end**, the **cue markers**, the **loop**, whose edges are drawn and dragged, and the **phrase boundary**, which is dragged to the beat a phrase starts on. The frame under the hand goes to djmanzo, which snaps it to the nearest beat, clamps it into the record and refuses an empty slot; the lane draws what comes back. It goes through the same action the pads send, so it lands in the session log and is remembered with the record — a moved boundary is stored as the DJ's own answer, which a later re-analysis may not overwrite. Still numbers in panels: saved loops, beat jumps, stem regions, AI suggestions |
