...
| 🟡 **5** | intelligence: context engine, transactions, takeover, promotion | **the posture matrix is enforced by test** — `dj_assistant::Warrant` makes the invalid cell unrepresentable rather than checked, and `no_posture_acts_on_a_read_something_disagrees_with` walks all six postures against all three certainties. The context engine ships (`dj_core::ContextEngine`) and is the common input to the attention budget, the theme's energy and the autopilot. Staged transactions and promotion are still to come |
