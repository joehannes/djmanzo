...
| 🟡 **5** | intelligence: context engine, transactions, takeover, promotion | **the gate is met** — the posture matrix is §72's own table in `dj_assistant::authority`, asserted against the directive verbatim and enforced as the second of three gates rather than drawn as a diagram. Transactions ship (`dj_app::staged`): the whole next transition staged with Accept, Modify and Reject, and a refused move shown greyed rather than dropped. Takeover was already per-parameter. The context engine ships and is the common input to the attention budget, the theme's energy and the autopilot. What is left of this phase is **promotion** — §41's typed UI vocabulary, so the assistant can ask for a surface rather than only for a control
