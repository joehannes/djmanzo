...
| Hot cues dragged on the waveform, snapping to the grid when quantise is on | M8 |
| Loops resized by dragging either edge on the waveform | M8 |
| Phrase boundaries dragged to the beat a phrase starts on | M8 |
| Saved loops drawn on the waveform, and played from there | M8 |
| The incoming record drawn over the outgoing lane, beatmatched | M8 |
| A contextual rail: the six controls this deck's moment calls for | M8 |
| The energy trajectory as a curve over the overview | M8 |
| One press that stops everything automatic, without touching the audio | M8 |
| A readable matrix of what the assistant may do at every posture | M8 |
| Breakdowns and drops, on the lane and the overview | M8 |
...
| **Cards**, the browser's second view: sleeves read out of the files' own tags, with the record's BPM, key and length under each and every gesture the table row has | Cockpit §20 |
...
| **Resize the loop by its edges** | **shipped** — a loop's length was a pair of buttons that halve and double it, which covers every length that is a power of two and no other. Both edges are drawn on the lane and dragged, on the same terms as a cue: quantise decides whether it snaps, it is clamped into the record, it will not turn the loop inside out, and a drag where no loop is running conjures nothing. The band moved to the bottom edge in the same change: it used to be a sixteen-per-cent wash across the lane, which tinted the spectral colouring underneath, so the audio inside a loop read as more mid-range than it is |
| **What the assistant may do, written out** | **shipped** — §72's override matrix. It was four booleans on `Posture`, and `may_act` answered one question for four different powers: nudging an EQ, riding sync and pulling the crossfader are not the same permission. One row per capability, one cell per posture, with `may_act`, `may_stage` and `may_mix` **derived from it** so there is one table rather than two free to disagree. Readable in the Conduct panel with the current posture marked rather than filtered to, because the question a DJ has is what changes if they turn the dial up. Rows for powers djmanzo does not have say "not yet" instead of showing empty cells that look like a setting. Not yet **configurable**: the cells are djmanzo's answer, not the DJ's to override |
| **Take over** | **shipped** — §47's emergency control, in the top bar beside REC and Mark. The section is one sentence long where it matters: *when something goes wrong, the user must not search through menus*, and the gesture existed only inside a panel you have to open. One press stops the automix, drops an assistant that was acting to `suggest`, and releases every control it could have moved. It **does not touch the audio** — no fader, no stop, no crossfader cut — because an emergency control that changes what a room can hear is worse than whatever it was pressed about, and there is no version of "safe" that is safe on every record. It says what it stopped: a press that appears to do nothing cannot be told from one that failed, and this is the control whose failure a DJ cannot afford to discover later |
| **The energy trajectory** | **shipped** — §25 asks for the energy trajectory as a *curve* rather than as two markers, and the two markers it already had — the breakdown band and the drop tick — are thresholds over exactly this measurement. So the line they came from is drawn on the overview, and the marks above it read as consequences rather than assertions. It is the drums' trajectory and not the record's loudness: the low band under 150 Hz, divided by the record's own ninetieth percentile, which is what makes a minimal techno record and a big-room house record comparable to themselves. A record that never loses its drums has one too — that answer used to be thrown away along with the empty breakdown list. On the overview and not the scrolling lane, because at the lane's zoom about ten beats are visible and a trajectory over ten beats is a straight line |
| **The contextual control rail** | **shipped** — §74's "core idea of adaptive UI": a compact strip on every loaded deck carrying the six controls the moment calls for. Four modes, and which one a deck is in comes from what the DJ has just done — a hand on the platter is scratching, a muted stem is a stem transition, a stopped deck is being readied, and a playing one is being mixed. That ordering is the safety rule as much as the lists are: every change follows an action they took, so the rail never rearranges itself under a hand reaching for it. Six, because that is what the attention budget allows while performing and sizing every mode to the tightest one means the rail does not shrink when the music starts. It does not latch — every control it promotes is also a widget on the deck, and that widget is where its state is shown. §74's named lists are followed where the vocabulary has a verb for them; stem FX is a continuous filter rather than a switch, and tags, rating and transition points are a library row and a panel, so those gaps are named rather than filled by inventing verbs for a strip of buttons |
| **The ghost track** | **shipped** — §27 asks for the future to be made visible: *if I bring this in here, this is what happens*, before committing. The pair view drew two records side by side, which answers what they are and not what they do together. The incoming record is now laid over the outgoing lane from the point the mix begins, at a zoom that makes one of its beats one of the outgoing's — drawn at its own frame rate it would visibly drift against a lane it is meant to be beatmatched to, and the ratio comes from Rust, which knows both tempos and both sample rates. No second player and no second decode: these are the incoming deck's own tiles, which already exist. See-through and not recoloured, because the ghost's colour is its own spectral balance exactly as the lane's is, and what separates them is solid against not. Nothing is drawn for a record with no phrase structure to come in on — a ghost at the top of a record would claim the mix brings in a beginning it does not. It carries the incoming record's own breakdown and drop too, drawn inside it: where the new record's drums leave and come back is the thing a DJ cannot see from the lane that is playing |
| **Saved loops are visible** | **shipped** — a loop kept with a record was invisible in both places it should not have been: the lane did not draw it, so a region cut from the audio could not be seen against that audio, and the pad page that recalls them lit no slot at all, so the only way to find out whether slot 5 held anything was to press it in front of a room. Both come from the same fact — a saved loop is the library's, and nothing carried it to the interface. The deck now remembers the set that arrived with its record, for the reason it already remembers the title, and it travels on the snapshot. On the lane a saved loop is a bracket along the bottom edge with its slot above the left end: hollow where the running loop is solid, because they are the same idea in two states and inventing a second colour for the second state is the overload §57 forbids. Pressing the slot plays it |
| **Move the phrase boundary** | **shipped** — the analyser can be right about how long a phrase is and wrong about which beat starts one; a record opening on a four-beat pickup is the usual cause, and a wrong anchor is not cosmetic because the planner, the automix, the autopilot and Set Flow all mix on it. The boundary is dragged on the deck's own lane to the beat it belongs on. Only the anchor moves: the grid is untouched, which is the opposite of every other grid edit and why it does not go down the path that clears the phrase. It snaps to the *nearest* beat rather than the one the drop is inside, and it is stored as the DJ's own answer, so the next load's analyser may no longer write over it — both of those were found by driving it rather than by a test, and the second meant a correction reverted on the next restart |
| **Grab the cue, do not re-cue** | **shipped** — a hot cue is dragged on the deck's own lane rather than set again at the playhead, which is what moving one used to mean. It snaps to a beat when quantise is on and not when it is off, because that is a question the DJ has already answered; it clamps into the record; and a slot with nothing in it does nothing rather than growing a cue where a gesture missed. It travels as the same action a controller's pad sends, so it lands in the session log and is remembered with the record |
| **A mix knows what it is running over** | **shipped** — a transition says how many of its beats have no drums under them, in the pair view's reasons and in the assistant's own sentence when the autopilot proposes one. It is a fact and not a veto: nothing moves a mix off a breakdown, because mixing in over the outgoing record's breakdown is a technique and mixing out into one is a mistake, and which of the two this is depends on what the DJ meant |
| **djmanzo sees the breakdown coming** | **shipped** — where the drums leave a record and where they come back, found per beat from the kick band measured against that record's own level rather than a fixed one, with hysteresis so a stray kick does not end a breakdown and a two-beat fill does not start one. Drawn on the scrolling lane and on the overview as a dim band along the top edge ending in a bright tick, which is deliberately not a wash over the waveform: the waveform's colour is the spectral balance, and tinting it is the overload §57 forbids. A record that fades out gets a breakdown and no drop, because inventing one would put a marker on the end of half the records ever pressed |
| **The browser has two views** | **shipped** — a table and a grid of cards, on the same rows: the same search, the same sort, the same selection, and the same actions, rendered from one snippet so the two cannot drift into one being the lesser browser. The sleeve is read out of the file's tags on demand and served over a `cover://` scheme as an ordinary image, so the browser fetches only what is on screen and decodes it off the main thread; a record with no picture — most of a real collection — falls back to its lettering rather than an empty square, and the lettering is underneath rather than instead of, so nothing flashes a broken-image icon while you scroll. Cards can be sorted from a control of their own, because a view you cannot order is the worse one. **Stage** puts a record on the deck that is not playing, which is what the automix and the autopilot both mean by somewhere to mix into; **favourite** writes five stars rather than a second flag that would mean the same thing and disagree |
