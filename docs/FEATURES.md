...
| **Share a set** | **shipped** — a night's tracklist handed to WhatsApp, X, Bluesky or Threads with the message already written and cut to what the network takes (a post on X holds a handful of records and says how many more), previewed first, and a file for the sets too long for any of them. Nothing is posted and no recipient is chosen: djmanzo prepares the share, the person presses send (§108) |
...
| **The night, read and shown** | **shipped** — `dj_core::ContextEngine` places the last few minutes in the whole night's own range of loudness and tempo, alongside whatever the DJ's occasion declares, and answers with a phase, a certainty and what produced it. It says nothing for the first six minutes and nothing at all on a night that has not varied, because a position inside a distribution that has not spread out is arithmetic rather than news. Where the two sources disagree the DJ wins, djmanzo says which way its own reading points, and the autopilot stops mixing unasked until it resolves. The **Night** surface shows all of it, including what it does not yet know |
| **The assistant asks before it acts** | **shipped** — a non-trivial move is a transaction: the whole next transition staged as one thing with Accept, Modify and Reject, in a strip that costs the decks no height until there is something to decide. Modify is a checkbox, because "load it and cue it, but I will bring it in myself" is the realistic change. A move your posture refuses is shown greyed and named rather than dropped, so the obvious fix is visible. Accepting runs the same code the automatic tick does, and partial success is reported as partial |
| **An override matrix, and three gates** | **shipped** — the directive's ten capabilities against six postures, verbatim. Before anything moves djmanzo asks three separate questions: has a hand claimed this control, does the posture permit this kind of thing, is the night sure enough to act on. Three rather than one because "it did nothing" deserves an answer. Configurable both ways, except that Off and Watch cannot be widened |
| **SAFE** | **shipped** — takes every control back, drops anything staged, clears every rack, flattens the tone, restores master gain and the limiter. It never stops a record, moves a fader or touches the crossfader: an emergency control that silences the floor is worse than the emergency. `safe` on the action bus, so it is on a controller pad and in a script too |
| **The assistant can ask for a panel** | **shipped** — `ui show prepare`, `ui pin room`, `ui focus 2`: a second closed vocabulary generated from the panels that exist, so one djmanzo does not have cannot be asked for. Never JavaScript, never the DOM. Your override matrix decides whether it may, and it is in the command palette for you too — pinning a panel had no gesture at all before this |
| **Grab the mix point** | **shipped** — the transition's start is a handle on the outgoing waveform. Drag it, or focus it and use the arrow keys. It moves in whole beats, because a mix point between two beats is not on the grid, and djmanzo re-derives what it now means: drag it off a phrase boundary and it stops claiming to land on one |
| **The mix you set up is the mix that happens** | **shipped** — hand over to the automix, or let the assistant run it, and it performs the transition djmanzo is holding: where you put it, over the length you chose, in the style you picked. Without one the handover is the end of the file minus the transition, which is wrong for any record with applause on the end. The panel says which of the two promises is in force, and dims the settings that are now about the *next* mix rather than this one |
| **The waveform says how long you have** | **shipped** — a wash over the last thirty seconds of a record, and the stretch a mix covers drawn as a region rather than two lines. Both are named layers in djmanzo's own inventory of the twenty §25 asks for: eleven exist, nine are named and not yet built, and a test refuses two of them sharing a colour meaning |
| **djmanzo remembers which records you put together** | **shipped** — *keep* on any of tonight's mixes saves that transition, directionally, with what the mix was; keeping it again says so more strongly. The Next rail then answers "why do I keep seeing these two together?" with the reason first on the line — strong enough to lift a candidate, capped so it cannot pin one to the top |
| **What you do, and when** | **shipped** — the gestures tonight's log contains, read through a rule that refuses to turn one surprising night into a preference: nothing generalises on fewer than four occurrences in one phase of the night, and every sentence names the phase it is about. "You sometimes sweep the filter when the night is building", never "you like filter sweeps" |
| **At hand** | **shipped** — the four to eight controls that matter on whichever deck your hands are on, changing with what you are doing: scratching, playing the stems, mixing against another record, or getting one ready. Each is an ordinary action, so a press is the same event as typing it or mapping a controller to it, and a latched control offers the other half rather than sitting beside it |
| **The collection as cards, with its own artwork** | **shipped** — sleeves read out of the files' tags and served as images on their own scheme, each card carrying its operations: deck, set aside, more like this, favourite, and why it was suggested. A record with no cover draws its key and tempo over a colour from the Camelot hour rather than a placeholder repeated down the grid |
| **The night knows its own mixes** | **shipped** — Tonight's mixes lists every handover in the set: what went into what, over how many beats, and whether it was a cut, a fade, a blend, an echo or a vocal drop. Derived from the action log rather than recorded beside it, so a set from before the feature existed has its mixes too, and one rule covers every way of doing it — the crossfader, the channel faders, or both |
| **One mix can be heard back on its own** | **shipped** — hear it again on any row of Tonight's mixes re-renders that handover to a WAV, with eight seconds of run-up and four of tail, from the set being played. Not a seek: the set is replayed up to the mix and discarded, because the engine's state at a moment is everything before it — so it takes a while on a long night, and the control says so |
| **The waveform says where the record can be left** | **shipped** — the window in which a mix out can begin, from the last beat where the longest transition djmanzo proposes still fits to the last beat where the shortest one does. Drawn over the whole record in the overview, where it can be seen from the moment the record loads, and to scale in the lane. Both edges come from the planner's own lengths and tail margin, so the band and its "rushed" warning cannot disagree |
| **The waveform says when the grid is a guess** | **shipped** — a hatch under a record whose beat grid the analyser is unsure of, over the record's own length. The rasteriser already draws the grid at the weight it deserves; at overview zoom it is suppressed for density, so faint and absent look the same, and this is the part that says which |
