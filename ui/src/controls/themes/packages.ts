...
export const AudioReactiveStroke: BehaviorModifier = (render, state, perf) => {
  ...
  const energy = state.context.energy_level || 0;
  
  return {
    ...render,
    paths: render.paths.map(p => ({
      ...p,
      stroke: `hsl(${220 - energy * 180}, 80%, 60%)`,
      strokeWidth: p.strokeWidth + (energy * 4),
...
export const AudioReactiveGlitch: BehaviorModifier = (render, state, perf) => {
  ...
  const treble = state.context.audio.spectral_bands[3] || 0;
...
export const TimeReactivePulse: BehaviorModifier = (render, state, perf) => {
  ...
  const bass = state.context.audio.spectral_bands[0] || 0;
...
export const NeonGlow: EffectProcessor = (render, state, perf) => {
...
export const ChromaticAberration: EffectProcessor = (render, state, perf) => {
  ...
  // Duplicate paths with RGB offsets
  const newPaths = [...render.paths];
  ...
  return { ...render, paths: newPaths };
};

export const ExclusionBlend: EffectProcessor = (render, state, perf) => {
  if (perf === "Eco") return render;
  return {
    ...render,
